...
control_genotypes=$( echo ${controls_genotypes} | tr '\t' ' ' | tr -s ' ' )
  exclude_animals=$( echo ${exclude} | tr '\t' ' ' | tr -s ' ' | awk '{print toupper($0)}' )
           filter=$( echo ${quality_filter} | tr '\t' ' ' | tr -s ' ' )
...
exclude_pseudogenes='FALSE' #In public version include pseudogenes always!
             header='TRUE'
	      start_dir=$( pwd )
         dir_output=$( echo "${start_dir}/${job_name}" )
        output_name=$( echo "${job_name}.vcf" )

            LOGFILE=$( echo "${job_name}.log" )
			
		 process_ID=$( echo $BASHPID )
		   temp_dir=$( echo "${start_dir}/temp_${process_ID}" )

echo "The process number is: ${process_ID}"
...
# make a temporary directory to do so:
rm -rf ${temp_dir}
mkdir ${temp_dir}
cd ${temp_dir}
...
echo '########################################################' | tee -a ${LOGFILE}
echo '# The actual analysis will start now - sit back, relax #' | tee -a ${LOGFILE}
echo '# and enjoy the computer working for you..             #' | tee -a ${LOGFILE}
echo '########################################################' | tee -a ${LOGFILE}
...
then
	...
	then
		...
		then 
			...
			sort -T ${temp_dir} variants_chromosomesDefined.vcf | uniq >> variants_severalChromosomes.vcf
	...
	then 
		...
		sort -T ${temp_dir} genesDefinded_getVariants.vcf | uniq >> variants_severalGenes.vcf 
...
echo '###############################################################' 
echo '# The first part is done. It might still take quite a while.. #' 
echo '# May I recommend something to read for you?                  #'
echo '# Georges, M., Charlier, C. and Hayes, B., 2018.              #'
echo '# Harnessing genomic information for livestock improvement.   #'
...
then 
	...
	sort -T ${temp_dir} variants_impactsDefined.vcf | uniq >> variants_severalImpacts.vcf
...
then 
	...
	sort -T ${temp_dir} variants_filterQulaitiesDefined.vcf | uniq >> variants_severalFilters.vcf
...
then
	...
	then
		...
		sort -T ${temp_dir} variants_definedTargetGenotypes.vcf | uniq >> variants_${labID_target}.vcf
...
#several target animals
	...
	echo ${target} | tr ' ' '\n' > definedTargets_temp
	cat header.vcf | tr '\t' '\n' | cat -n | sort -T ${temp_dir} -k2,2 | join -o'1.1' -1 2 -2 1 - <(sort -T ${temp_dir} -k1,1 definedTargets_temp) > positionsOfDefinedTargets_temp
	...
	sort -T ${temp_dir} tempfile_ExcludingNotWantedVariants.vcf | uniq >> variants_sevTar.vcf
...
then
	...
	# check for several genotypes
		...
		sort -T ${temp_dir} variants_definedControlsGenotypes.vcf | uniq >> variants_"${labID_control}".vcf
...
#several control animals
	...
	cat header.vcf | tr '\t' '\n' | cat -n | sort -T ${temp_dir} -k2,2 | join -o'1.1' -1 2 -2 1 - <(sort -T ${temp_dir} -k1,1 definedControls_temp) > definedControlsColumns_temp
	...
	sort -T ${temp_dir} variants_ExcludingNonControlsGenotypes_temp | uniq >> variants_sevCon.vcf
...
echo "### Now the fun part starts!! ###"  | tee -a ${LOGFILE}
echo "Some final statistics for you: "  | tee -a ${LOGFILE}
echo "The total number of unique variants is: "  | tee -a ${LOGFILE}
cut -d' ' -f1-4 ${selectionIII} | sort -T ${temp_dir} | uniq | wc -l  | tee -a ${LOGFILE}
echo -e "\n" | tee -a ${LOGFILE}

echo "The number of unique HIGH impact variants is: "  | tee -a ${LOGFILE}
grep "HIGH" ${selectionIII} | cut -d' ' -f1-4 | sort -T ${temp_dir} | uniq | wc -l  | tee -a ${LOGFILE}
echo -e "\n" | tee -a ${LOGFILE}

echo "The number of unique MODERATE impact variants is: "  | tee -a ${LOGFILE}
grep "MODERATE" ${selectionIII} | cut -d' ' -f1-4 | sort -T ${temp_dir} | uniq | wc -l  | tee -a ${LOGFILE}
echo -e "\n" | tee -a ${LOGFILE}

echo "The number of unique LOW impact variants is: "  | tee -a ${LOGFILE}
grep "LOW" ${selectionIII} | cut -d' ' -f1-4 | sort -T ${temp_dir} | uniq | wc -l  | tee -a ${LOGFILE}
echo -e "\n" | tee -a ${LOGFILE}

echo "The number of unique MODIFIER impact variants is: "  | tee -a ${LOGFILE}
grep "MODIFIER" ${selectionIII} | cut -d' ' -f1-4 | sort -T ${temp_dir} | uniq | wc -l  | tee -a ${LOGFILE}
echo -e "\n" | tee -a ${LOGFILE}

echo "!!! Remember that the numbers will most probably not sum up, as a variant can be predicted with several impacts !!!" | tee -a ${LOGFILE}
...
echo '###################################################################################' | tee -a ${LOGFILE}
echo '# We are looking forward to seeing you again on our tour threw the animal genome! #' | tee -a ${LOGFILE}
echo '###################################################################################' | tee -a ${LOGFILE}
